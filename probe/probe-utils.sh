...
function generate_annotations_context(){
  ...
  if [ -n "${unhealthy_pod_name[0]}" ];then
...
function inject_annotations(){
  ...
  if [ -n "$increase_pod_num" ];then
    local current_pod_num=`grep -n 'replicas' $dc_file_path |head -1 | cut -d ":" -f 3 | cut -d "," -f 1`
    local new_pod_num=$(($current_pod_num + $increase_pod_num))
    local replicas_line_num=`grep -n 'replicas' $dc_file_path |head -1 | cut -d ':' -f 1`
    sed -i "${replicas_line_num}s#    \"replicas\":.*#    \"replicas\": ${new_pod_num},#" $dc_file_path
  fi
}
