...
increase_pod_num=$(find_env "INCREASE_POD_NUM")
max_isolate_num=$(find_env "MAX_ISOLATE_NUM" 2)
block_isolate_temp=$(find_env "BLOCK_ISOLATE" "false")
...
if [ "$response_code" != "200" ];then
   echo "[Fatal]: Can't request API,Please check policy!"
   exit 200
fi
...
if [ ! "$current_unhealthy_label" ];then  #normal pod (not be isolated, to be checked)
   if [ "$pod_current_state" == "false" ];then #not ready --> ready
      if [ -f ${check_script_path}/startup-check.sh ];then
         /bin/sh ${check_script_path}/startup-check.sh
         if [ $? != 0 ];then
      ...
      if [ -f ${check_script_path}/running-check.sh ];then
         /bin/sh ${check_script_path}/running-check.sh
         if [ $? == 0 ];then
            echo 0 > /tmp/$times_file_name
            exit 0
         else
      ...
      if [ "$current_times" -ge "$max_failure_times" ];then
         ...
         while (( i++ < retry_times ));do
            ...
            inject_annotations "$annotations_context" "$dc_file_path" "$increase_pod_num"
   ...
   exit 1
    ...
    echo "[Info]: $pod_name is unhealthy"
    exit 1
fi
